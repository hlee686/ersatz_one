...
export const BELabs = () => {
  ...
  const list: { label: string; onClick: () => void }[] = [
    ...
    },
    {
      ...
      onClick: async () => console.log(await getUserInfoByCustomId(state)),
    },
    {
      label: "가짜 글 생성",
    ...
    },
    {
      ...
      onClick: async () =>
        await editUserInfo({
          profileImage:
            "https://firebasestorage.googleapis.com/v0/b/velog-copy.appspot.com/o/images%2FtestPath?alt=media&token=f86169c1-7b85-4b1f-91b8-ec755ccb5a45",
          email: "test@test.com",
          name: "변경된 이름",
          introduction: "변경된 소개",
          velogTitle: "변경된 벨로그",
  ...
  return (
    <>
      ...
      <div style={{ display: "flex", gap: "10px", flexWrap: "wrap" }}>
        <input
          placeholder="input"
          className={cc(["w-[100px] h-[100px] border-2 border-black"])}
          value={state}
      ...
      <input
        ...
        onChange={async (e) => {
          ...
          if (file) {
            const result = await handleImageUpload(file, "testPath");
